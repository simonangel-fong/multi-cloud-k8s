...
> Origin endpoints (EKS ELB hostname, AKS LB IP) come from the Envoy Gateway `gateway.status.addresses[0].value` in each cluster — see [argocd.md](argocd.md). Wired into `cloudflare` via `tfvars` for now; promote to cross-stack `terraform_remote_state` once stable.
